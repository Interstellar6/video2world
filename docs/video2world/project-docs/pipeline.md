...
执行收据比计划更严格：每轮 `input_clean_plate.sha256` 必须等于上一轮 `output_clean_plate.sha256`，并且每轮都有重新 scene audit、SAM3、质量审核和新 clean plate 的内容 hash。object round 必须按 `target_id` 绑定对象补全 receipt，receipt targets 必须与本轮 `target_ids` 完全一致；它输出的 clean plate 只能声明 `acceptance_scope="corrected_clean_plate_next_round_source_only"` 与 `lineage_scope="corrected_clean_plate_round_source"`，同时 `corrected_full_pipeline=false`、`promotion_approved=false`、`canonical_promotion_approved=false`，只能喂给下一轮 segmentation/SAM/reinspection。除 clean plate 链路本身以外，scene audit、SAM3、quality、object completion 与 background rebuild receipt 的 hash 必须彼此独立，不能跨角色或跨轮复用同一个 artifact。末轮必须是 `final_background` 并绑定 background rebuild receipt；只有末轮 output 与 `final_clean_plate` 才能声明 `acceptance_scope="corrected_full_pipeline"` 和 canonical promotion approval。同批 `provider_receipt` 还必须 hash 绑定 layered stage 的全部上游输入和全部输出资产，包括 `semantic_gaussian`、`object_facts`、`completed_object_assets_manifest`、`final_clean_plate` 与 `clean_scene_mesh`，并对 JSON/PLY snapshot 执行语义校验：canonical input 角色路径和内容 hash 不能互相复用；`semantic_gaussian` 必须有 Gaussian 字段、`object_id` 和 `object_probability`，且不能复用 raw `scene_gaussian` 输入的 hash 或路径；clean scene PLY 必须有可验证的 Gaussian/mesh 结构，terminal report 必须显式绑定 clean_scene_gaussian/clean_scene_mesh 资产，且其 uri/hash/size 必须与 receipt output 一致；completed object asset IDs 与 scene_id/run_id 必须等于 plan/report planned targets 和 terminal report，final clean plate 必须同时匹配 report、clean plate manifest 与 receipt output snapshot，provider receipt 输出角色路径和内容 hash 不能互相复用，clean scene Gaussian/mesh 必须是不同于 raw scene inputs 的新 artifact，clean scene Gaussian 也不能复用 semantic_gaussian 输入的 hash 或路径。缺少深层对象、真实背景、provider receipt、任一中间轮次，输入/输出 artifact 类型或语义字段不对，复用 raw/semantic scene input 或 execution receipt，object receipt/completed assets/final clean plate manifest 与 target、report 或真实输出不匹配，或把 next-round source 伪装成 terminal report 时，`layered_completion_report` adapter 直接拒绝，不能靠首层候选、旧 demo manifest 或手写 status 过关。
