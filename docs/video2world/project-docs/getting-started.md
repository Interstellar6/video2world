...
同批 `provider_receipt.inputs` 必须内容寻址绑定完整 layered stage 输入：`frames_manifest`、`cameras`、`layered_completion_plan`、`scene_gaussian`、`scene_mesh`、`masks_manifest`、`captions_manifest`、`object_clouds_manifest`、`semantic_gaussian`、`object_facts`，且 canonical input 角色路径和内容 hash 不能互相复用。`provider_receipt.outputs` 也必须绑定完整输出：`completed_object_assets_manifest`、`clean_scene_gaussian`、`clean_scene_mesh`、`clean_plate_manifest`、`layered_completion_report`。adapter 会重新 hash 每个 snapshot path，并读取对应资产验证 JSON 可解析、Gaussian/Mesh PLY 头字段齐全；terminal report 必须显式绑定 clean_scene_gaussian/clean_scene_mesh 资产并与 receipt 输出的 uri/hash/size 一致；`semantic_gaussian` 必须同时带 Gaussian 字段、`object_id` 和 `object_probability`，且不能复用 raw `scene_gaussian` 输入 hash 或路径；`completed_object_assets_manifest.objects[*].id` 与 scene_id/run_id 必须与 executed plan/report 的 planned targets 和 terminal report 一致，`clean_plate_manifest.final_clean_plate` 必须与 report 的 terminal `final_clean_plate` 一致，provider receipt 输出角色路径和内容 hash 不能互相复用，`clean_scene_gaussian`/`clean_scene_mesh` 不能复用原始 `scene_gaussian`/`scene_mesh` 输入 hash 或路径，`clean_scene_gaussian` 也不能复用 `semantic_gaussian` 输入。缺任一角色、hash/size/file_count 不一致、语义字段缺失，或 receipt 指向另一份 report/plan/clean scene 资产，都会 fail closed。
