...
`layered_completion` 的 terminal report 只有在全部 front-to-back rounds 完成时才通过：report 必须显式声明 `lineage_scope="corrected_full_pipeline"`，并且必须和同批 `provider_receipt` 一起提交，以绑定真实执行过的 completion plan SHA-256；round 必须连续消费上一轮 clean plate hash，每轮都绑定新的 scene-audit receipt、SAM3 receipt、quality report 与 output clean plate；object round 还要有对象建模 receipt，末轮必须是带真实 background rebuild receipt 的 `final_background`。`current_demo_only`、archived Web promotion、旧四轮 sequence、无 provider receipt 的 report 或单个 front pillow 候选都不能满足这个合同。
