...
collision-enabled 的 unified PBR GLB 不能只写 `status=passed`。World manifest 要求 `alignment`、`collision` 与 `visual` 三个 gate 的 passed 状态都带 `report_uri`、`report_sha256` 与 `report_size_bytes`，分别指向 scene fit / 支撑穿模审核、BVH collision 审核和六视图或浏览器 visual QA。`video2world validate` 会重新 hash 本地 gate report，并要求 report 是非空 JSON object；对 collision-enabled unified PBR GLB，这些 report 还必须显式写入 `kind`、`gate`、`status`、`scoped_id`、`object_id` 或 `target_id`，以及 `asset_sha256`、`source_asset_sha256`、`unified_pbr_glb_sha256` 或 `collision_asset_sha256` 中至少一个当前 GLB hash 绑定字段。`kind` 必须是 `video2world.unified_gate.alignment`、`video2world.unified_gate.collision` 或 `video2world.unified_gate.visual` 中与 gate 对应的值；所有显式身份、状态和资产 hash 都必须分别和 manifest gate 名称、状态、对象身份及当前 unified PBR GLB hash 一致，report 中的 `metrics` 也必须等于对应 `quality_gates.<gate>.metrics`。passed gate 的标准阻断指标不能自相矛盾：例如明显/严重穿模、严重身份漂移、空白浏览器画布、少于六视图、零 BVH probe 或不稳定支撑接触都会被拒绝。alignment report 还必须绑定当前 `bbox_scene` 与 `transform_scene_from_asset`；collision report 必须绑定当前 `collision_topology` 与 unified GLB provenance 中的 face count；visual report 必须提供 `front/right/back/left/top/bottom` 六视图的 evidence object，且每个视角都要有非空 `uri` 和 64 位小写十六进制 `sha256`。这样对象进入 Web 交互层之前，pivot、摆放、碰撞和视觉审核都有可复算证据，而不是手写通过。
